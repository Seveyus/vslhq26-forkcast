...
description: >
  ...
  accent, the three-step gray text ladder, Segoe UI Variable Display (display/numerals/chrome) + Segoe UI Variable Display (body),
...
colors:
  ...
  accent-medium: "rgba(74, 158, 255, 0.15)"
  border: "rgba(74, 158, 255, 0.2)"
  card-bg: "rgba(74, 158, 255, 0.04)"
  positive: "#35D39A"
  negative: "#FF7089"
...
typography:
  # — reading ramp (Segoe UI Variable Display body + Segoe UI Variable Display chrome) —
  body:    { fontFamily: "Segoe UI Variable Display", cqw: 0.85, weight: 400, lineHeight: 1.6, color: "text-muted" }
  h4-eyebrow:{ fontFamily: "Segoe UI Variable Display", cqw: 0.8, weight: 600, tracking: "0.08em", upper: true, color: "primary" }
  tag:     { fontFamily: "Segoe UI Variable Display", px: 12, weight: 550, color: "primary" }
  counter: { fontFamily: "Segoe UI Variable Display", px: 13, weight: 550, tracking: "0.05em", color: "text-muted" }
  # — display / numerical ramp (Segoe UI Variable Display, near-black headings / cobalt numerals) —
  h3:      { fontFamily: "Segoe UI Variable Display", cqw: 1.25, weight: 550, lineHeight: 1.3, tracking: "-0.02em", color: "text" }
  stat-num:{ fontFamily: "Segoe UI Variable Display", cqw: 1.9, weight: 660, lineHeight: 1.0, color: "primary" }
  blockquote:{ fontFamily: "Segoe UI Variable Display", cqw: 2.4, weight: 550, lineHeight: 1.35, color: "text" }
  h2:      { fontFamily: "Segoe UI Variable Display", cqw: 2.6, weight: 600, lineHeight: 1.1, tracking: "-0.02em", color: "text" }
  metric-value:{ fontFamily: "Segoe UI Variable Display", cqw: 3.0, weight: 660, lineHeight: 1.0, color: "primary" }
  h1:      { fontFamily: "Segoe UI Variable Display", cqw: 4.2, weight: 660, lineHeight: 1.08, tracking: "-0.02em", color: "text" }
  quote-mark:{ fontFamily: "Segoe UI Variable Display", cqw: 8.0, weight: 660, lineHeight: 0.5, color: "primary", opacity: 0.15 }
...
components:
  ...
  cta-button:
    ...
    textColor: "{colors.bg}"
    rounded: "{radii.pill}"
    typography: "Segoe UI Variable Display 600"
...
- **Fonts** — already set to **Segoe UI Variable Display** (display) / **Segoe UI Variable Display** (body); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{400, 550, 580, 600, 620, 640, 650, 660}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
The voice is two faces in fixed roles: **Segoe UI Variable Display** (display, every numeral, all chrome —
eyebrows uppercase 0.08em) and **Segoe UI Variable Display** (body, muted gray, line 1.6). Headlines are near-black;
...
- **Segoe UI Variable Display** (display/numerals/chrome) + **Segoe UI Variable Display** (body) — near-black headlines, cobalt numerals.
...
Two ramps. The **reading ramp** (Segoe UI Variable Display body 0.85cqw muted; Segoe UI Variable Display eyebrow uppercase 0.08em
cobalt) carries copy + chrome; the **display/numerical ramp** (Segoe UI Variable Display `h3` 1.25cqw → `h1`
...
- **Headlines near-black, −0.02em**; **eyebrows cobalt, uppercase, 0.08em**; **numerals cobalt 600–700**; **body Segoe UI Variable Display 400 muted, line 1.6**. No italic, no uppercase body, no cobalt headline.
...
**Composes** accent-line, meta, h1, body sub. **Focal** a 2-line Segoe UI Variable Display `h1` near-black, left,
...
**Focal** a row of tinted cards — cobalt `metric-value` + Segoe UI Variable Display label + muted desc + optional
...
blockquote, cite. **Focal** a Segoe UI Variable Display `blockquote` near-black under a 15%-opacity cobalt
...
Segoe UI Variable Display body column beside a cobalt-8% highlight block (4px cobalt left rule) carrying an inline pull
...
a Segoe UI Variable Display `h1` near-black, centered, with the one solid cobalt `cta-button` pill below. **Accent**
...
- Use **tinted cards** (4% fill, 20% border, 10–14px radius, no shadow); body Segoe UI Variable Display 400 muted, line 1.6.
...
- **Type** — Segoe UI Variable Display headings −0.02em near-black, cobalt eyebrows 0.08em + numerals; Segoe UI Variable Display body muted line 1.6; ≥1.4cqw floor.
...
- **Segoe UI Variable Display + Segoe UI Variable Display via Google Fonts.** CJK pairing (Noto Sans SC 700 display / Noto Serif SC 400 body) carries over; the eyebrow's uppercase+tracking signal weakens in CJK — pair it with the accent-line.
