...
// Get to a decision without spending take time on it.
await page.getByRole('button', { name: 'Simulate response options' }).click()
await page.waitForSelector('.film', { timeout: 60_000 })
mark('decision ready')

// Frame the film section, then play it for real.
await page.locator('.film').scrollIntoViewIfNeeded()
await page.waitForTimeout(700)
await clickAt(page.getByRole('button', { name: 'Play decision film' }))
mark('film playing')

// Let the scenes and the evidence rail advance together.
await page.waitForTimeout(14_500)
mark('film mid-playback')

// Now change the assumption, so the take shows the film go stale and come back.
await clickAt(page.getByRole('button', { name: 'Test assumption' }))
await page.waitForSelector('.delta', { timeout: 60_000 })
mark('counterfactual applied')

await page.locator('.film').scrollIntoViewIfNeeded()
await page.waitForTimeout(1200)
await clickAt(page.getByRole('button', { name: /Play decision film|Restart/ }))
mark('film replayed on new evidence')
...
if (video) {
  ...
  const target = path.join(outDir, 'film-journey.webm')
